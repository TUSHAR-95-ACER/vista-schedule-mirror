...
DROP TRIGGER IF EXISTS weekly_plans_touch_updated_at ON public.weekly_plans;
...
BEGIN
  ...
  IF p_expected_revision IS NOT NULL AND p_expected_revision <> v_existing.revision THEN
    INSERT INTO public.plan_audit_log(user_id, plan_id, plan_type, operation, success, error, payload_size, payload_hash, revision)
      VALUES (v_user, p_id, 'daily', 'update', false,
        ...
        v_payload_size, v_payload_hash, v_existing.revision);
  ...
  SELECT COALESCE(MAX(version), 0) + 1 INTO v_history_version
    FROM public.daily_plan_history WHERE plan_id = v_existing.id;
  ...
  RETURN jsonb_build_object(
    'revision', v_revision,
    'previous_revision', v_existing.revision,
    'inserted', false,
    'updated_at', v_saved.updated_at,
    'row_count', v_rows,
    'history_created', true,
    'history_version', v_history_version,
    'payload_hash', v_payload_hash
  );
END;
$function$;
...
BEGIN
  ...
  SELECT COALESCE(MAX(version), 0) + 1 INTO v_history_version
    FROM public.weekly_plan_history WHERE plan_id = v_existing.id;
